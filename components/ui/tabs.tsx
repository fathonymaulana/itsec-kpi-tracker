  ...
  "group/tabs-list inline-flex w-fit max-w-full items-center justify-center overflow-x-auto scrollbar-hide rounded-lg p-[3px] text-muted-foreground group-data-horizontal/tabs:h-8 group-data-vertical/tabs:h-fit group-data-vertical/tabs:flex-col data-[variant=line]:rounded-none data-[variant=pill]:rounded-full",
  {
    variants: {
      variant: {
        ...
        // The app's standard segmented-pill tab style — rounded-full outer container, active tab
        // gets a solid bg-primary (near-black in light mode) fill rather than the default variant's
        // bg-background + shadow treatment.
        pill: "gap-1 rounded-full border border-divider bg-panel p-1 group-data-horizontal/tabs:h-10",
      },
    },
    defaultVariants: {
      variant: "default",
...
function TabsTrigger({ className, ...props }: TabsPrimitive.Tab.Props) {
  return (
    <TabsPrimitive.Tab
      ...
      className={cn(
        // flex-1 alone (flex: 1 1 0%) lets a trigger shrink below its own label's width once the
        // list runs out of room — exactly what let "Modify Requests" clip instead of scrolling on a
