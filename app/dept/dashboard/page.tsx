...
// Same formatting as formatValue, but with the unit split out — the KPI Status cards put the number
// and its unit at different sizes/weights (a big bold value, a smaller muted suffix), which a single
// combined string can't do.
function formatValueParts(v: number | null, unit: string): { value: string; suffix: string } {
...
// (a 0-1 fraction for '%' units, same as everywhere else in this file), so it gets the same *100
// scaling formatValue applies before display. `improving` flips for direction=-1 KPIs (lower is
// better, e.g. cost/attrition/error-rate metrics) — a negative delta there is the good direction.
function formatDelta(deltaRaw: number | null, unit: string, direction: number): { text: string; improving: boolean } | null {
...
export default function DeptDashboard() {
  ...
  const kpisWithData = kpis.map(kpi => {
    ...
    const { value: currentV, status: currentStatus } = statusFor(kpi, actualsByYearMonth[rangeTo.year] || {}, rangeTo.month)
    // Target/direction come from the PRIMARY sub-metric, not kpi.numeric_target/direction directly —
    // those are stale for KPIs with multiple independently-targeted components (see statusFor above).
    const primary = getPrimarySubMetric(kpi.sub_metrics)
    ...
    // "vs last month" reads the second-to-last point already computed above — the range's own
    // second-to-last month, not necessarily calendar-last-month, so it stays correct when a narrower
    // range is picked. Null (not shown) if the range doesn't have a prior point to compare against.
    ...
    return { kpi, unit, monthValues, hasData, currentV, currentStatus, period, target, direction, pctOfTarget, deltaRaw }
  })
            ...
            >
              <Tabs value={view} onValueChange={v => v && setView(v as 'charts' | 'table')}>
                ...
                <TabsContent value="charts" className="space-y-4">
                  {/* One at-a-glance overview card for the whole department — status cards instead of
                      the small-multiples sparkline grid this used to be: current value, target,
                      progress-to-target bar, and month-over-month delta, all colored by that KPI's
                      current status. The full-size trend charts further below still carry the shape-
                      over-time job; these are meant for a faster "who needs attention right now" scan
                      than a dozen sparklines require parsing individually. */}
                            ...
                            >
                              ...
                              {target !== null && (
                                <div className="text-xs text-ink-muted mb-3">
                                  Target: {formatValueParts(target, unit).value}{formatValueParts(target, unit).suffix}
                                </div>
                              )}
                              {clampedPct !== null && (
                              ...
                              {delta && (
                                <div className="flex items-center gap-1 text-xs font-medium" style={{ color: delta.improving ? 'var(--success-text)' : 'var(--danger-text)' }}>
                                  {delta.improving ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
